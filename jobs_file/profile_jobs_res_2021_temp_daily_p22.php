<?php 
	...
	$script = 'SELECT * FROM `t_profiling_ub_res` WHERE pid = 1 AND flag = 2';
	$querys =  mysqli_query($con,$script); 
	while($row_users = mysqli_fetch_array($querys)){
		
		$id = $row_users['id'];
		$user_id_profil = $row_users['user_id_profil'];
		echo $id."\n";
		
		//$periode_a = strtoupper(date_format(date_create(date('Y-m-d')),"Y-m")); //2018-11
		//$periode_a = "2022-November"; //2018-11
		$periode_a = "2023-04"; //2018-11
		
		$script_checkS = "DELETE FROM M_MONTH_PROFILE_RES_P22 WHERE PERIODE = '".$periode_a."' AND PROFILE_ID = ".$id;   
		$query_checkS =  mysqli_query($con,$script_checkS);  
		
		
		$sql5 = "INSERT INTO M_MONTH_PROFILE_RES_P22 VALUES('".$periode_a."','','',".$id.",'0') ";
		...
		$periode_list_script = "SELECT PERIODE FROM M_MONTH_PROFILE_RES_P22 WHERE PROFILE_ID = ".$id." AND STATUS_PROCESS = 0 AND PERIODE = '".$periode_a."' ORDER BY PERIODE DESC ";   
				 ...
				 INSERT INTO PTV_CIM_RATING_RES
											...
											SELECT * FROM (												
												...
												(
													...
													LEFT JOIN (SELECT IF(CHANNEL = 'ONE','S-ONE',CHANNEL) AS CHANNEL_NAME,
