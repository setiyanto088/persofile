<?php 
	...
	$periode_list_script = "SELECT PERIODE FROM M_MONTH_PROFILE_RES_P22 WHERE PROFILE_ID = ".$id." AND STATUS_PROCESS = 0 ORDER BY PERIODE DESC ";   
	...
	foreach($arr_periode as $arp){
		...
		$percentage = $percentage + (1/$int_prog);
		$script_doneb_s = "UPDATE t_profiling_ub_res SET global_progress = '".$percentage." %' where id ='".$id."' ";
		mysqli_query($con,$script_doneb_s); 
		...
		$percentage = $percentage + (2/$int_prog);
		$script_doneb_s = "UPDATE t_profiling_ub_res SET global_progress = '".$percentage." %' where id ='".$id."' ";
		mysqli_query($con,$script_doneb_s);  
...
IF(CAST(RC.SPLIT_MINUTES AS TIME) > CAST('00:00:00' AS TIME) AND CAST(RC.SPLIT_MINUTES AS TIME) < CAST('06:00:00' AS TIME) ,'00:00 - 06:00',
	...
	";	
		...
		$percentage = $percentage + (4/$int_prog);
		$script_doneb_s = "UPDATE t_profiling_ub_res SET global_progress = '".$percentage." %' where id ='".$id."' ";
		//mysqli_query($con,$script_doneb_s);  
		...
		$percentage = $percentage + (12.5/$int_prog);
		$script_doneb_s = "UPDATE t_profiling_ub_res SET global_progress = '".$percentage." %' where id ='".$id."' ";
		//mysqli_query($con,$script_doneb_s);  
...
AND ID_PROFILE = ".$id."
		...
		$percentage = $percentage + (0.5/$int_prog);
		$script_doneb_s = "UPDATE t_profiling_ub_res SET global_progress = '".$percentage." %' where id ='".$id."' ";
		//mysqli_query($con,$script_doneb_s);  
	...
	$commands = 'php /var/www/jobs/profile_jobs_res_p22_temp_ext.php '.$id.' > /var/www/jobs/log_profile_n_'.$id.'res_2021.log 2>&1 & ';  
