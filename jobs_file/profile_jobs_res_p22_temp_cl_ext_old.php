	...
	";	
		...
		INSERT INTO `M_SUM_TV_DASH_CHAN_RES_FULL`
						...
						A.ID_PROFILE,1 AS STS FROM (
							SELECT CHANNEL,SUM(WEIGHT) AS AUDIENCE,SUM(WEIGHT) AS AUDIENCE_ALL,PERIODE,'' HARIS,'AUDIENCE' AS DT,0 AS IDPRO, STS,".$id." ID_PROFILE  FROM (
								SELECT RESPID,CHANNEL,WEIGHT,'".$periode."' PERIODE, 0 STS  FROM `CDR_EPG_RES_".$name_tbs_new."_STEP2_2022`
		...
		INSERT INTO `M_SUM_TV_DASH_CHAN_DAY_RES_FULL`
						...
						A.IDPRO,1 AS STS FROM (
								...
								SELECT RESPID,CHANNEL,WEIGHT,'".$periode."' PERIODE,formatDateTime(BEGIN_PROGRAM,'%Y-%m-%d') HARIS, 0 STS  FROM `CDR_EPG_RES_".$name_tbs_new."_STEP2_2022`
								WHERE RESPID IN (SELECT `CARDNO` AS people FROM PROFILE_CARDNO_RES WHERE M_TYPE = 0 AND ID_PROFILE = ".$id." )
								GROUP BY RESPID,CHANNEL,WEIGHT,formatDateTime(BEGIN_PROGRAM,'%Y-%m-%d')
							...
							WHERE PERIODE = '".$periode."'
							AND PROFILE_ID = '".$id."'
							GROUP BY CHANNEL,formatDateTime(SPLIT_MINUTES,'%Y-%m-%d')
		...
		INSERT INTO `M_SUM_TV_DASH_PROG_RES_FULL`
						...
						A.ID_PROFILE,1 AS STS,0 as tpe FROM (
							SELECT CHANNEL,PROGRAM,SUM(WEIGHT) AS AUDIENCE,SUM(WEIGHT) AS AUDIENCE_ALL,PERIODE,'' HARIS,'AUDIENCE' AS DT,0 AS IDPRO, STS,".$id." ID_PROFILE  FROM (
								SELECT RESPID,PROGRAM,CHANNEL,WEIGHT,'".$periode."' PERIODE, 0 STS  FROM `CDR_EPG_RES_".$name_tbs_new."_STEP2_2022`
								WHERE RESPID IN (SELECT `CARDNO` AS people FROM PROFILE_CARDNO_RES WHERE M_TYPE = 0 AND ID_PROFILE = ".$id.")
								GROUP BY RESPID,PROGRAM,CHANNEL,WEIGHT
		...
		INSERT INTO `M_SUM_TV_DASH_PROG_DAYE_RES_FULL`
						...
						A.IDPRO,1 AS STS,0 as tpe FROM (
								...
								SELECT RESPID,CHANNEL,PROGRAM,WEIGHT,'".$periode."' PERIODE,formatDateTime(BEGIN_PROGRAM,'%Y-%m-%d') HARIS, 0 STS  FROM `CDR_EPG_RES_".$name_tbs_new."_STEP2_2022`
								WHERE RESPID IN (SELECT `CARDNO` AS people FROM PROFILE_CARDNO_RES WHERE M_TYPE = 0 AND ID_PROFILE = ".$id." )
								GROUP BY RESPID,CHANNEL,PROGRAM,WEIGHT,formatDateTime(BEGIN_PROGRAM,'%Y-%m-%d')
		...
		INSERT INTO `M_SUM_TV_DASH_PROG_RES_FULL`
						...
						A.ID_PROFILE,1 AS STS,1 as tpe FROM (
							SELECT CHANNEL,CONCAT(PROGRAM,' ',toString(BEGIN_PROGRAM)) PROGRAM,SUM(WEIGHT) AS AUDIENCE,SUM(WEIGHT) AS AUDIENCE_ALL,PERIODE,'' HARIS,'AUDIENCE' AS DT,0 AS IDPRO, STS,".$id." ID_PROFILE  FROM (
								SELECT RESPID,PROGRAM,BEGIN_PROGRAM,CHANNEL,WEIGHT,'".$periode."' PERIODE, 0 STS  FROM `CDR_EPG_RES_".$name_tbs_new."_STEP2_2022`
								WHERE RESPID IN (SELECT `CARDNO` AS people FROM PROFILE_CARDNO_RES WHERE M_TYPE = 0 AND ID_PROFILE = ".$id.")
								GROUP BY RESPID,PROGRAM,BEGIN_PROGRAM,CHANNEL,WEIGHT
							) P GROUP BY CHANNEL,PROGRAM,BEGIN_PROGRAM,PERIODE,STS
						) A JOIN (
							SELECT `CHANNEL`,CONCAT(PROGRAM,' ',toString(BEGIN_PROGRAM)) PROGRAM,
							AVG(VIEWERS) AS VIEWERS, AVG(VIEWERS_A) AS VIEWERS2,
							AVG(TVR)*100 AS TVR,AVG(TVR_A)*100 AS TVR2,
							AVG(TVS)*100 AS TVS,AVG(TVS_A)*100 AS TVS2
							FROM `SUMMARY_PER_MINUTES_RES_V2`
		...
		INSERT INTO `M_SUM_TV_DASH_PROG_DAYE_RES_FULL`
						...
						A.IDPRO,1 AS STS,0 as tpe FROM (
								...
								SELECT RESPID,CHANNEL,PROGRAM,BEGIN_PROGRAM,WEIGHT,'".$periode."' PERIODE,formatDateTime(BEGIN_PROGRAM,'%Y-%m-%d') HARIS, 0 STS  FROM `CDR_EPG_RES_".$name_tbs_new."_STEP2_2022`
								WHERE RESPID IN (SELECT `CARDNO` AS people FROM PROFILE_CARDNO_RES WHERE M_TYPE = 0 AND ID_PROFILE = ".$id." )
								GROUP BY RESPID,CHANNEL,PROGRAM,BEGIN_PROGRAM,WEIGHT,formatDateTime(BEGIN_PROGRAM,'%Y-%m-%d')
