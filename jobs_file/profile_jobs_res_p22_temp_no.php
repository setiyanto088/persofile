<?php 
	...
	$script_checkDF = "SELECT COUNT(*) AS CNT FROM M_MONTH_PROFILE_RES_P22 WHERE STATUS_PROCESS = 3"; 
	 $query_checkDF =  mysqli_query($con,$script_checkDF);  
	$check_progressDF = mysqli_fetch_array($query_checkDF);

	//print_r($check_progressDF[0]['CNT']);die;

	if($check_progressDF[0]['CNT'] == 0){
  ...
  if($check_progressDF[0]['CNT'] == 0){
		   ...
		   $scriptFR = " 	SELECT * FROM M_MONTH_PROFILE_RES_P22 WHERE STATUS_PROCESS = 2 
					ORDER BY PERIODE DESC
					LIMIT 1 "; 
			 $queryFR =  mysqli_query($con,$scriptFR);  
			$check_progressFR = mysqli_fetch_array($queryFR);
	
			$id = $check_progressFR[0]['PROFILE_ID'];
